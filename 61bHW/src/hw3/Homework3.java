...
public class Homework3 {
  ...
  public static void smoosh(int[] ints) {
	  for (int i = 0; i < ints.length; i++){
		  for(int k = i+1; k < ints.length && ints[i] == ints[k]; k++){
			  ints[k] = -1;
		  }
	  }
  ...
  //'empty' tests
    int[] test100 = {1};
    System.out.println("smooshing " + stringInts(test100) + ":");
    smoosh(test100);
    result = stringInts(test100);
    ...
    int[] test101 = {1,1,1,1,1,1,2,3,3,3,3,3,3,4,5,5,5};
    SList list101 = new SList();
    for (i = 0; i < test101.length; i++) {
      list101.insertEnd(new Integer(test101[i]));
    }
    System.out.println("squishing " + list101.toString() + ":");
    list101.squish();
    result = list101.toString();
    System.out.println(result);
    TestHelper.verify(result.equals("[  1,2,3,4,5  ]"),
