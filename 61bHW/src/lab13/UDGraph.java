...
public class UDGraph
{
    	...
    	for (int j = 0; j<this.vertices; j++){
    		if(this.adjMatrix[i][j]){
    			for (int a = 0; a < vertices; a++){
    				if (this.adjMatrix[j][a]){
    					newGraph.adjMatrix[i][a] = true;
    					newGraph.edges++;
    				}
    			}
    		}
  ...
  public UDGraph paths(int length) {
    ...
    if (length == 2){
	    ...
	    UDGraph lenMinOne = this.paths(length-1);
	    for (int i = 0; i < lenMinOne.vertices; i++){
	    	for (int j = 0; j< lenMinOne.vertices; j++){
	    		if (lenMinOne.adjMatrix[i][j]){
	    			for (int a = 0; a < lenMinOne.vertices; a++){
	    				if (this.adjMatrix[j][a]){
		    				newGraph.addEdge(i, a);
	    				}
	    			}
	    		}
	    	}
