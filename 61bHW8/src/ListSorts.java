...
public class ListSorts {
  ...
  public static void mergeSort(LinkedQueue q) {
    // Your solution here.
	  try{  
		  LinkedQueue merge = makeQueueOfQueues(q);
		  while (merge.size()!=1){
			  LinkedQueue q1 = (LinkedQueue)merge.dequeue();
			  LinkedQueue q2 = (LinkedQueue)merge.dequeue();
			  LinkedQueue combinedQueue = mergeSortedQueues(q1,q2);
			  merge.enqueue(combinedQueue);
		  }
		  q.append((LinkedQueue)merge.dequeue());
	  } catch(QueueEmptyException e){
		  System.out.println(e);
	  }
  }
  ...
  public static void quickSort(LinkedQueue q) {
    // Your solution here.
	  if (q.size()!=0){
		  int random = 1+(int)((q.size())*Math.random());
