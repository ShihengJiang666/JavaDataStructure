...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    Wall[] myWall = new Wall[horiz*(vert-1)+(horiz-1)*vert];
     ...
     **/
    for(int w = myWall.length-1;w>0;w--){
        temp = myWall[w];
        int rand = randInt(w);
        myWall[w] = myWall[rand];
        myWall[rand] = temp;
      }

      for(int w=0;w<myWall.length;w++){
        if (myWall[w].type=="h"){
          int x = myWall[w].x;
          int y = myWall[w].y;

          int root1 = cells.find(y*horiz+x);
          int root2 = cells.find((y+1)*horiz+x);
          if(root1!=root2){
            cells.union(root1, root2);
            hWalls[x][y]=false;
          }
        }
        else if (myWall[w].type=="v"){
          int x = myWall[w].x;
          int y = myWall[w].y;

          int root1 = cells.find(y*horiz+x);
          int root2 = cells.find(y*horiz+x+1);
          if(root1!=root2){
            cells.union(root1, root2);
            vWalls[x][y]=false;
          }
